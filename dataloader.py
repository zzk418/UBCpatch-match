...
def matchMaker(dataDir, DB):
    ...
    labels = np.genfromtxt(os.path.join(dataDir, DB, "labels.csv"), delimiter=',')
    ...
    tmp_nonmatches = np.concatenate(([imgsAdrsList[(split+lenLeft//2):len(imgsAdrsList)]], [imgsAdrsList[split:(split+lenLeft//2)]]), axis=0)
...
def DBcombiner(DBs):
    ...
    for db in DBs:
        ...
        if os.path.exists(TEMPS_DIR+db+"_matches.csv"):  #read from saved files if already available
            ...
            np.savetxt(db+"_matches.csv", matchSet, delimiter=",", fmt='%s')
            np.savetxt(db+"_nonmatches.csv", nonmatchSet, delimiter=",", fmt='%s')
    ...
    np.savetxt("All_matches.csv", AllMatches, delimiter=",", fmt='%s')
    np.savetxt("All_nonmatches.csv", AllNonMatches, delimiter=",", fmt='%s')
